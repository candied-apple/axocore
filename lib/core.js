...
async function downloadAll({ version = minecraft.MINECRAFT_VERSION, destDir }) {
  ...
  const progressCb = (info) => {
    ...
    if (info.type === 'client') {
      ...
      msg = `[${fileCount}] Downloading asset ${info.index}/${info.total}: ${path.basename(info.file)}`;
    }
...
async function launch({
  ...
  // Always check/repair core Minecraft files first
  const { clientJar, libraries, assetsDir } = await downloadAll({ version, destDir: resolvedDestDir });
