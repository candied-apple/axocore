...
async function getVersionManifest() {
  ...
  const res = await axios.get(MINECRAFT_MANIFEST);
  return res.data;
}
...
async function getVersionInfo(version) {
  ...
  const axios = require('axios');
  const res = await axios.get(versionObj.url);
  return res.data;
}
...
async function downloadAssets(version, destDir, progressCb) {
  ...
  for (const [name, obj] of Object.entries(objects)) {
    ...
    if (needDownload) {
      if (progressCb) progressCb({ type: 'asset', file: assetPath, index: idx, total });
      fs.mkdirSync(path.dirname(assetPath), { recursive: true });
      const url = `https://resources.download.minecraft.net/${subdir}/${hash}`;
      await require('./downloader').downloadFile(url, assetPath, false);
    }
  }
