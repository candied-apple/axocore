...
function getLogFilePath(gameDir) {
  // Always resolve logs relative to the main Minecraft directory (not gameDir)
  // If gameDir is .../mcdata/game, logs should be .../mcdata/logs
  const mcRoot = path.dirname(gameDir);
...
function getCrashReportPath(gameDir) {
  // Always resolve crash-reports relative to the main Minecraft directory (not gameDir)
  const mcRoot = path.dirname(gameDir);
